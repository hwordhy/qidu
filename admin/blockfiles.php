...
case "blocks":
	...
	include_once jieqi_path_inc("class/blocks.php", "system");
    ...
    $jieqiTpl->assign("pubdir", jieqi_htmlstr($_REQUEST["pubdir"]));
	...
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/blockblocks.html", "system");
...
case "edit":
	...
	if ($blockSet["custom"] == 1) {
		...
		if (!empty($blockSet["filename"]) && preg_match("/^\w+$/", $blockSet["filename"])) {
			$jieqiBlockset = array();
            $file = jieqi_path_config($blockSet["filename"], $blockSet["module"]);
			$file = @realpath($file);
    ...
    $blocks_form->addElement(new JieqiFormHidden("pubdir", $_REQUEST["pubdir"]));
...
case "update":
	...
	if ($_POST["cacheupdate"] == 1) {
		...
		if ($blockSet["custom"] == 1) {
			include_once jieqi_path_inc("blocks/block_custom.php", "system");
		}
		else if (preg_match("/^\w+$/is", $filename)) {
			include jieqi_path_inc("blocks/" . $filename . ".php", $modname);
		}
	...
	jieqi_jumppage(JIEQI_URL . "/admin/blockfiles.php?act=blocks&module=" . urlencode($_REQUEST["module"]) . "&filename=" . urlencode($_REQUEST["filename"]) . "&pubdir=" . $_REQUEST["pubdir"], LANG_DO_SUCCESS, $jieqiLang["system"]["block_update_success"]);
	break;
...
default:
    ...
    $jieqiTpl->assign("pubdir", $_REQUEST['pubdir']);
	$jieqiTpl->setCaching(0);
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/blockfiles.html", "system");
	include_once jieqi_path_common("admin/footer.php");
	break;
}
