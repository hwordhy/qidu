...
define('MYSQL_SERVER_INFO', mysqli_get_server_info($query_handler));
if ($_REQUEST['option'] == 'export') {
	if (isset($_POST['act']) && ($_POST['act'] == 'backup')) {
		...
		if ($exporttype == 'all') {
			...
			if (is_array($exporttables) && (0 < jieqi_count($exporttables))) {
		...
		if (empty($errtext)) {
			...
			if ($exportmode == 'multivol') {
				...
				while (1) {
					...
					for (; strlen($sqldump) < ($sizelimit * 1000); $tableid++) {
	...
	if (isset($_POST['act']) && ($_POST['act'] == 'recover')) {
		...
		if (empty($errtext)) {
			...
			if (is_array($sqlfilearray) && (0 < jieqi_count($sqlfilearray))) {
		...
		if (isset($_POST['act']) && ($_POST['act'] == 'delete') && is_array($_POST['checkid']) && (0 < jieqi_count($_POST['checkid']))) {
			jieqi_checkpost();
			jieqi_getconfigs(JIEQI_MODULE_NAME, 'backuplog');
		...
		if (is_array($jieqiBackuplog) && (0 < jieqi_count($jieqiBackuplog))) {
...
$jieqiTset['jieqi_contents_template'] = jieqi_path_template("admin/dbmanage.html", "system");
...
function jieqi_sqldumptable($table, $startfrom = 0, $currsize = 0)
{
	...
	if (!$query) {
		...
		if ($fieldrow = $query_handler->db->fetchArray($query)) {
	...
	if ($extendins == '0') {
		while ((($currsize + strlen($tabledump)) < ($sizelimit * 1000)) && ($numrows == $offset)) {
			...
			while ($row = $query_handler->db->fetchRow($rows)) {
				...
				for (; $i < $numfields; $i++) {
					$t .= $comma . ($usehex && !empty($row[$i]) && (jieqi_strexists($tablefields[$i]['Type'], 'char') || jieqi_strexists($tablefields[$i]['Type'], 'text')) ? '0x' . bin2hex($row[$i]) : '\'' . mysqli_real_escape_string($rows, $row[$i]) . '\'');
					$comma = ',';
				}
		...
		while ((($currsize + strlen($tabledump)) < ($sizelimit * 1000)) && ($numrows == $offset)) {
			...
			if ($numrows = $query_handler->db->getRowsNum($rows)) {
				...
				while ($row = $query_handler->db->fetchRow($rows)) {
					...
					for (; $i < $numfields; $i++) {
						$t2 .= $comma2 . ($usehex && !empty($row[$i]) && (jieqi_strexists($tablefields[$i]['Type'], 'char') || jieqi_strexists($tablefields[$i]['Type'], 'text')) ? '0x' . bin2hex($row[$i]) : '\'' . mysqli_real_escape_string($rows, $row[$i]) . '\'');
						$comma2 = ',';
					}
...
function jieqi_splitsqlfile(&$ret, $sql, $release = 32270)
{
	...
	for (; $i < $sql_len; ++$i) {
		...
		else if (($release < 32270) && ($char == '!') && (1 < $i) && ($sql[$i - 2] . $sql[$i - 1] == '/*')) {
			$sql[$i] = ' ';
		}
	}
...
function jieqi_getfilesarray($basename)
{
	...
	if (is_array($filearray) && (0 < jieqi_count($filearray))) {
		$i = 0;

		for (; $i < jieqi_count($filearray); $i++) {
