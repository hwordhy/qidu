...
if ((!empty($_REQUEST["id"]) || !empty($_POST["groupname"])) && (0 < jieqi_count($groupary))) {
	jieqi_setconfigs("groups", "jieqiGroups", $groupary, "system");
	$publicdata = str_replace("?><?php", "", jieqi_readfile(jieqi_path_config("system")) . jieqi_readfile(JIEQI_ROOT_PATH . "/lang/define.php") . jieqi_readfile(jieqi_path_config("groups")));
	jieqi_writefile(jieqi_path_config("define"), $publicdata);
}

$jieqiTpl->setCaching(0);
$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/groups.html", "system");
include_once jieqi_path_common("admin/footer.php");
