...
{
	$file = jieqi_path_config("modules");
	$data = "<?php\r\n";
...
$fileroot = JIEQI_ROOT_PATH . "/templates/themes";
$handle = opendir($fileroot);
$themes = array();
while (($handle !== false) && (($file = readdir($handle)) != false)) {
	if (($file[0] != ".") && is_dir($fileroot . "/" . $file)) {
...
$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/managemodules.html", "system");
