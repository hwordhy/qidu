...
include_once jieqi_path_inc("class/right.php", "system");
...
if (0 < jieqi_count($jieqiRight[$_REQUEST["mod"]])) {
	if (isset($_POST["act"]) && ($_POST["act"] == "update")) {
		...
		include_once jieqi_path_inc("class/honors.php", "system");
		...
		unset($criteria);
		$right_form = new JieqiThemeForm($jieqiLang["system"]["edit_right"], "right", JIEQI_URL . "/admin/right.php?mod=".$_REQUEST["mod"]);

		foreach ($jieqiRight[$_REQUEST["mod"]] as $k => $v ) {
			$tmpvar = "";

			foreach ($honors as $honor ) {
                $tmpvar .= "<div style=\"margin:5px auto;\">";
				$right_text = new JieqiFormText($honor["caption"], $k . "[" . $honor["honorid"] . "]", 20, 60, $v["honors"][$honor["honorid"]]);
				$tmpvar .= $right_text->getCaption() . " " . $right_text->render() . "</div>";
			}
