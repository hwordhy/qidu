...
function jieqi_api_bindcheck()
{
	...
	if (!empty($_SESSION["jieqiUserApi"][$apiName]["unionid"])) {
		$sql = "SELECT * FROM " . jieqi_dbprefix("system_userapi") . " WHERE " . jieqi_dbslashes($apiField) . " = '" . jieqi_dbslashes($_SESSION["jieqiUserApi"][$apiName]["openid"]) . "' OR wxunid = '" . jieqi_dbslashes($_SESSION["jieqiUserApi"][$apiName]["unionid"]) . "' LIMIT 0, 1";
	}
	else {
	...
	if (is_array($apirow) && !empty($apirow["uid"])) {
		...
		if (!is_object($jieqiUsers)) {
			...
			if (!empty($_SESSION["jieqiUserApi"][$apiName]["unionid"]) && empty($apirow[$apiField])) {
				...
				$sql = "UPDATE " . jieqi_dbprefix("system_userapi") . " SET apidata = '" . jieqi_dbslashes(serialize($apidata)) . "' WHERE uid = " . intval($apirow["uid"]);
				$query->execute($sql);
			}
		}
	}
...
function jieqi_api_binduser($params = array())
{
	...
	if ($userrow["avatar"] == 0) {
		...
		if (!empty($uinfo["url_avatar"]) && preg_match("/^https?:\/\//i", $uinfo["url_avatar"])) {
			...
			$imgdata = jieqi_urlcontents($uinfo["url_avatar"], array("charset" => "image", "connecttimeout" => 3, "timeout" => 3));
...
function jieqi_api_loginjump($user, $ucsyncode = "")
{
	...
	if (empty($_SESSION["jieqiUserApi"][$apiName]["jumpurl"])) {
		...
		unset($_SESSION["jieqiUserApi"][$apiName]["jumpurl"]);
	}
