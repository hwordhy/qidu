...
function jieqi_device_check(){
	if(jieqi_ismobile()){
		//手机访问情况
		if(defined('JIEQI_MOBILE_LOCATION') && strlen(trim(JIEQI_MOBILE_LOCATION)) > 0 && defined('JIEQI_LOCAL_URL') && ('http://' . $_SERVER['HTTP_HOST'] != JIEQI_MOBILE_LOCATION)){
		...
		//电脑访问情况
		if(defined('JIEQI_PC_LOCATION') && strlen(trim(JIEQI_PC_LOCATION)) > 0 && defined('JIEQI_LOCAL_URL') && ('http://' . $_SERVER['HTTP_HOST'] != JIEQI_PC_LOCATION)){
...
function jieqi_device_route_mobile(){
	if(!defined('JIEQI_MOBILE_LOCATION')) return false;
...
function jieqi_device_route_pc(){
	if(!defined('JIEQI_PC_LOCATION')) return false;
