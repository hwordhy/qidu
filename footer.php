...
if (empty($jieqiTset["jieqi_page_template"])) {
	$jieqiTpl->display(jieqi_path_template(JIEQI_THEME_NAME."/theme.html", "themes"));
}
else {
	if (($jieqiTset["jieqi_page_template"][0] != "/") && ($jieqiTset["jieqi_page_template"][1] != ":")) {
		if (strpos($jieqiTset["jieqi_page_template"], "/") === false) {
			if (defined("JIEQI_THEME_ROOTNEW") && is_file(JIEQI_THEME_ROOTPATH . "/themes/" . JIEQI_THEME_NAME . "/" . $jieqiTset["jieqi_page_template"])) {
				$jieqiTpl->display(jieqi_path_template(JIEQI_THEME_NAME . '/' . $jieqiTset['jieqi_page_template'], "themes"));
			}
			else {
				$jieqiTpl->display(jieqi_path_template(JIEQI_THEME_NAME . '/' . $jieqiTset['jieqi_page_template'], "themes"));
			}
...
if (!empty($_GET["fromuid"]) && defined("JIEQI_PROMOTION_VISIT") && ((0 < JIEQI_PROMOTION_VISIT) || (0 < JIEQI_PROMOTION_REGISTER))) {
