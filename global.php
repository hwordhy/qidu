...
define('JIEQI_COVER_ROOT', __DIR__ . '/cover');

if (!defined('JIEQI_COVER_DIR')) {
    $tmpvar = 'wap';
    if (preg_match('/^([\\w\\-]+)\\.[\\w\\-]+\\.(com|cn|net|com\\.cn|net\\.cn|top|vip|club|online)(:\\d+)?$/i', $_SERVER['HTTP_HOST'], $matches) && is_dir(JIEQI_COVER_ROOT . '/' . $matches[1])) {
        define('JIEQI_COVER_DIR', $matches[1]);
    }
    else if ($jieqi_is_mobile = jieqi_ismobile() && is_dir(JIEQI_COVER_ROOT . '/m')) {
        define('JIEQI_COVER_DIR', 'm');
    }
    else if (is_dir(JIEQI_COVER_ROOT . '/' . $tmpvar)) {
        define('JIEQI_COVER_DIR', $tmpvar);
    }
}

if (!defined('JIEQI_COVER_DIR')) {
    define('JIEQI_COVER_DIR', '');
}


if (!defined('JIEQI_ROOT_PATH')) {
    @define('JIEQI_ROOT_PATH', __DIR__);
}

include_once jieqi_path_config('define');
...
if (!defined('JIEQI_SYSTEM_CHARSET')) {
    if (defined('JIEQI_DB_CHARSET') && (JIEQI_DB_CHARSET == 'gbk')) {
        @define('JIEQI_SYSTEM_CHARSET', 'gbk');
    }
    else {
        @define('JIEQI_SYSTEM_CHARSET', 'utf-8');
    }
}
if (function_exists('mb_internal_encoding')) {
    mb_internal_encoding(strtoupper(JIEQI_SYSTEM_CHARSET));
}

if ((isset($_SERVER['HTTP_X_REQUESTED_WITH']) && (strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')) || !empty($_REQUEST['ajax_request'])) {
    define('JIEQI_AJAX_REQUEST', 1);
}
else {
    define('JIEQI_AJAX_REQUEST', 0);
}
...
if (!defined('JIEQI_DEVICE_FOR')) {
    $tmpvar = 'pc';
    if (defined('JIEQI_MOBILE_LOCATION') && (0 < strlen(JIEQI_MOBILE_LOCATION)) && (parse_url(JIEQI_MOBILE_LOCATION, PHP_URL_HOST) == $_SERVER['HTTP_HOST'])) {
        define('JIEQI_DEVICE_FOR', 'mob');
    }
    else if (defined('JIEQI_PC_LOCATION') && (0 < strlen(JIEQI_PC_LOCATION)) && (parse_url(JIEQI_PC_LOCATION, PHP_URL_HOST) == $_SERVER['HTTP_HOST'])) {
        define('JIEQI_DEVICE_FOR', 'pc');
    }
    else if ((JIEQI_BROWSER_NAME == 'weixin') || !empty($jieqi_is_mobile) || jieqi_ismobile()) {
        define('JIEQI_DEVICE_FOR', 'mob');
    }
    else {
        define('JIEQI_DEVICE_FOR', 'pc');
...
if (empty($jieqi_license_modules) || ((JIEQI_LOCAL_HOST == '') && (JIEQI_PHP_CLI == 1) && (ALLOW_PHP_CLI == 1)) || preg_match('/^' . preg_quote(str_replace(array('\\\\', '\\'), '/', JIEQI_ROOT_PATH), '/') . '\\/(admin|install|logout\\.php)/is', JIEQI_SCRIPT_FILENAME) || preg_match('/^(http:\\/\\/|https:\\/\\/)?[^\\/\\?]*(localhost|127.0.0.1)/i', JIEQI_LOCAL_HOST, $matchs)) {
	...
	// 	$tmpvar = md5($jieqi_license_ary[1] . $jieqi_license_ary[2] . 'yscy' . chr(74) . '19' . JIEQI_TARGET_TOP . 'dd' . chr(72) . 'dd16', true);
...
$jieqiModules = array();
include_once jieqi_path_config('modules');
...
foreach ($jieqiModules as $k => $v) {
    if (empty($v['dir'])) {
        $jieqiModules[$k]['dir'] = '/modules/' . $k;
    }

    if (empty($v['url'])) {
        $jieqiModules[$k]['url'] = ($k == 'system' ? JIEQI_URL : JIEQI_URL . $jieqiModules[$k]['dir']);
    }

    if (empty($v['themeurl'])) {
        $jieqiModules[$k]['themeurl'] = $jieqiModules[$k]['url'];
    }
...
if (!defined('JIEQI_CHAR_SET')) {
	if (defined('JIEQI_CHARSET_CONVERT') && (JIEQI_CHARSET_CONVERT == 1) && (JIEQI_VERSION_TYPE != '') && (JIEQI_VERSION_TYPE != 'Free')) {
		...
		else if (isset($_COOKIE['jieqiUserCharset']) && isset($jieqi_charset_type[$_COOKIE['jieqiUserCharset']])) {
...
else if ((strpos(JIEQI_CACHE_DIR, '/') === false) && (strpos(JIEQI_CACHE_DIR, DIRECTORY_SEPARATOR) === false)) {
...
else if ((strpos(JIEQI_COMPILED_DIR, '/') === false) && (strpos(JIEQI_COMPILED_DIR, DIRECTORY_SEPARATOR) === false)) {
...
if (!empty($_COOKIE[$sess_name]) || !empty($_REQUEST[$sess_name]) || defined('JIEQI_NEED_SESSION')) {
	...
	if (JIEQI_SESSION_STORAGE == 'db') {
		include_once jieqi_path_inc("include/session.php", "system");
		$sess_handler = &JieqiSessionHandler::getInstance('JieqiSessionHandler');
		@(session_set_save_handler(array(&$sess_handler, 'open'), array(&$sess_handler, 'close'), array(&$sess_handler, 'read'), array(&$sess_handler, 'write'), array(&$sess_handler, 'destroy'), array(&$sess_handler, 'gc')));
	}
	else if ((JIEQI_SESSION_SAVEPATH != '') && is_dir(JIEQI_SESSION_SAVEPATH)) {
	...
	if (!empty($_COOKIE[$sess_name]) && !empty($_COOKIE['jieqiUserInfo']) && (jieqi_count($_SESSION) == 0)) {
...
$parsed = @parse_url(trim(JIEQI_CACHE_DIR));
$scheme = (empty($parsed['scheme']) ? '' : $parsed['scheme']);

switch ($scheme) {
    case 'memcached':
    case 'memcache':
        $jieqiCache = &JieqiCache::getInstance('memcached', $parsed);
    case 'redis':
        $jieqiCache = &JieqiCache::getInstance('redis', $parsed);
    default:
...
if (empty($_SESSION['jieqiUserId'])) {
	if (!empty($_REQUEST['jieqi_username']) && !empty($_REQUEST['jieqi_userpassword'])) {
		session_start();
		include_once jieqi_path_inc("include/checklogin.php", "system");
	...
	else if (!empty($_REQUEST['luid']) && !empty($_REQUEST['lups'])) {
		session_start();
		include_once jieqi_path_inc("include/checklogin.php", "system");
...
else if (defined('JIEQI_IS_OPEN') && (JIEQI_IS_OPEN == 2) && !defined('JIEQI_ADMIN_LOGIN') && ($jieqiUsersStatus != JIEQI_GROUP_ADMIN) && (0 < jieqi_count($_POST))) {
	header('Content-Type:text/html; charset=' . JIEQI_CHAR_SET);
	echo LANG_DENY_POST;
...
class JieqiCache extends JieqiObject
{
    ...
    static public function getInstance($type = false, $options = [])
    {
        if (in_array(strtolower($type), ['file', 'memcached', 'redis'])) {
...
class JieqiCacheMemcached extends JieqiCache
{
    ...
    public function get($name, $ttl = 0, $over = 0)
    {
        $key = ($this->_md5key ? md5($name) : $name);
    ...
    public function set($name, $value, $ttl = 0, $over = 0)
    {
        ...
        $this->_mc->set($name . $this->_keyext, time(), 0, $ttl) && $this->_mc->set($name, $value, 0, $ttl);
    ...
    public function delete($name)
    {
        ...
        $this->_mc->delete($name . $this->_keyext) && $this->_mc->delete($name);
...
class JieqiCacheRedis extends JieqiCache
{
    ...
    public function set($name, $value, $ttl = 0, $over = 0)
    {
        ...
        if (0 < $ttl) {
            $this->_co->setx($name . $this->_keyext, $ttl, time()) && $this->_co->setx($name, $ttl, $value);
            return $this->_co->setx($name . $this->_keyext, $ttl, time()) && $this->_co->setx($name, $ttl, $value);
        }
        else {
            $this->_co->set($name . $this->_keyext, time()) && $this->_co->set($name, $value);
    ...
    public function delete($name)
    {
        ...
        $this->_co->delete($name . $this->_keyext) && $this->_co->delete($name);
...
//2018.09.04 引入PHP文件 @$file = 路径文件名
function jieqi_path_common($file = 'header.php')
{
    return JIEQI_ROOT_PATH . '/' . $file;
}

//2018.09.04 引入lib文件 @$file = 路径文件名
function jieqi_path_lib($file)
{
    return JIEQI_ROOT_PATH . '/lib/' . $file;
}

//2018.09.04 引入inc文件 @$file = 路径文件名
function jieqi_path_inc($file, $module = '')
{
    if (empty($module)) {
        return $file;
    }
    else {
        return $file == '' ? JIEQI_ROOT_PATH . '/modules/' . $module : JIEQI_ROOT_PATH . '/modules/' . $module . '/' . $file;
    }
}

//2018.09.04 PC+移动端自适应匹配配置文件 @$fname = 配置文件名 @$module = 模组
function jieqi_path_config($fname, $module = 'system')
{

    if (JIEQI_COVER_DIR != '' && is_file($tmpvar = JIEQI_COVER_ROOT . '/' . JIEQI_COVER_DIR . '/configs/' . $module . '/' . $fname . '.php')) {
        return $tmpvar;
    }
    else {
        return JIEQI_ROOT_PATH . '/configs/' . $module . '/' . $fname . '.php';
    }
}

//2018.09.04 PC+移动端自适应引入lang提示 @$fname = 配置文件名 @$module = 模组
function jieqi_path_lang($fname = '', $module = 'system')
{
    $tmpvar = strrpos($fname, '/');

    if (0 < $tmpvar) {
        $path = substr($fname, 0, $tmpvar + 1);
        $file = substr($fname, $tmpvar + 1);
        $fname = (substr($file, 0, 1) == '_' ? $path . 'lang' . $file . '.php' : $path . 'lang.php');
    }
    else {
        $fname = (substr($fname, 0, 1) == '_' ? 'lang' . $fname . '.php' : 'lang.php');
    }

    return JIEQI_ROOT_PATH . '/lang/' . $module . '/' . $fname;
}

//2018.09.04 PC+移动端自适应匹配模板文件 @$file = 模板文件路径 @$module = 模组
function jieqi_path_template($file = '', $module = false)
{
    ...
    if ($module !== false) {
        $tplfile = JIEQI_ROOT_PATH . '/templates';
    ...
    if ((JIEQI_COVER_DIR != '') && is_file($tmpvar = str_replace(JIEQI_ROOT_PATH, JIEQI_COVER_ROOT . '/' . JIEQI_COVER_DIR, $file))) {
        $file = $tmpvar;
    }
    return $file;
}

function jieqi_ajax_retout($data, $code = 0, $type = 'data')
{
    header('Content-Type:text/html; charset=' . JIEQI_CHAR_SET);
...
function jieqi_jumppage($url, $title = '', $content = '', $direct = NULL)
{
    ...
    if(!empty($_POST['ajax_gets']) || !empty($_REQUEST['ajax_gets'])) {
        echo json_encode(array("success" => true, "message" => $content, "backUrl" => jieqi_headstr($url)));
    }
...
function jieqi_msgbox($title, $content)
{
    if(!empty($_REQUEST['ajax_gets'])) {
        echo json_encode(array("success" => true, "message" => $content, "backUrl" => ''));
    }else
	if (empty($_REQUEST['ajax_request'])) {
		...
		$jieqiTpl->setCaching(0);
		return $jieqiTpl->fetch(jieqi_path_template(JIEQI_THEME_NAME . "/msgbox.html", "themes"));
	}
	else {
		header('Content-Type:text/html; charset=' . JIEQI_CHAR_SET);
		header('Cache-Control:no-cache');
...
function jieqi_msgwin($title, $content, $setting = array())
{
    ...
    if(!empty($_REQUEST['ajax_gets'])) {
        echo json_encode(array("success" => true, "message" => $content, "backUrl" => '', 'setting' => $setting));
    }
    else if (empty($_REQUEST['ajax_request'])) {
		include_once jieqi_path_lib("template/template.php");
		$title = jieqi_htmlstr($title);
		$jieqiTpl = &JieqiTpl::getInstance();
		$jieqiTpl->assign(array('jieqi_themecss' => JIEQI_THEME_ROOTURL . '/themes/' . JIEQI_THEME_NAME . '/style.css', 'title' => $title, 'content' => $content, 'jieqi_sitename' => JIEQI_SITE_NAME));
		$jieqiTpl->setCaching(0);
		$jieqiTpl->display(jieqi_path_template(JIEQI_THEME_NAME . "/msgwin.html", "themes"));
	}
	else {
		header('Content-Type:text/html; charset=' . JIEQI_CHAR_SET);
		header('Cache-Control:no-cache');
...
function jieqi_printfail($errorinfo)
{
    ...
    if(!empty($_REQUEST['ajax_gets'])) {
        echo json_encode(array("success" => false, "message" => $errorinfo, "backUrl" => ''));
    }else
	if (empty($_REQUEST['ajax_request'])) {
        include_once jieqi_path_lib("template/template.php");
		$jieqiTpl = &JieqiTpl::getInstance();
		$jieqiTpl->assign(array('jieqi_themecss' => JIEQI_THEME_ROOTURL . '/themes/' . JIEQI_THEME_NAME . '/style.css', 'errorinfo' => $errorinfo, 'debuginfo' => $debuginfo, 'jieqi_sitename' => JIEQI_SITE_NAME));
		$jieqiTpl->setCaching(0);
		$jieqiTpl->display(jieqi_path_template(JIEQI_THEME_NAME . "/msgerr.html", "themes"));
	}
	else {
		header('Content-Type:text/html; charset=' . JIEQI_CHAR_SET);
		header('Cache-Control:no-cache');
...
function jieqi_geturl($module, $target)
{
	...
	if (!function_exists($funname) && isset($jieqiModules[$module]['path']) && is_file(jieqi_path_inc("include/funurl.php", $module))) {
		include_once jieqi_path_inc("include/funurl.php", $module);
	}
...
function jieqi_checkpower($powerset = array(), $ustatus = '0', $ugroup = '0', $isreturn = false, $isadmin = false)
{
	...
	if ($ustatus == JIEQI_GROUP_ADMIN) {
	...
	else if (is_array($powerset['groups']) && (in_array($ugroup, $powerset['groups'], false) || in_array('0', $powerset['groups'], false))) {
		if ($isadmin && empty($_SESSION['jieqiAdminLogin'])) {
			if ($isreturn) {
				return false;
			}
        ...
        if(!empty($_REQUEST['ajax_gets'])) {
            return json_encode(array("success" => true, "message" => LANG_NEED_LOGIN, "backUrl" => $url));
        }else
		if (empty($_REQUEST['ajax_request'])) {
			if ($isadmin) {
...
function jieqi_checklogin($isreturn = false, $isadmin = false)
{
	...
	if ($isreturn) {
        ...
        if(!empty($_REQUEST['ajax_gets'])) {
            echo json_encode(array("success" => false, "login" => false, "message" => LANG_NEED_LOGIN, "backUrl" => ""));
            //echo "<script language=\"javascript\" type=\"text/javascript\">pop_login();</script>";
        }else
		if (empty($_REQUEST['ajax_request'])) {
			if (empty($_POST)) {
				$local_domain_url = (empty($_SERVER['HTTP_HOST']) ? '' : 'http://' . $_SERVER['HTTP_HOST']);
...
function jieqi_setusersession($user)
{
    ...
    $vipid = intval(jieqi_getvipid($user->getVar('experience'), $jieqiVips));
    $_SESSION['jieqiUserVipId'] = $vipid;
    $_SESSION['jieqiUserVipName'] = $jieqiVips[$vipid]['caption'];
...
function jieqi_includedb()
{
	if (!defined('JIEQI_DBCLASS_INCLUDE')) {
		include_once jieqi_path_lib("database/database.php");
...
{
    if (defined('JIEQI_DB_CONNECTED') && !defined('JIEQI_DB_NOTCLOSE') && !JIEQI_DB_PCONNECT) {
        JieqiDatabase::close();
    }
}
...
function jieqi_loadlang($fname = '', $module = 'system', $vname = 'jieqiLang')
{
    global ${$vname};
    include_once jieqi_path_lang($fname, $module);
    return ${$vname};
}
...
function jieqi_ismobile()
{
	...
	else if (isset($_SERVER['HTTP_VIA']) && (stripos($_SERVER['HTTP_VIA'], 'wap') !== false)) {
		return true;
	}
	else if (isset($_SERVER['HTTP_USER_AGENT']) && preg_match('/(blackberry|configuration\\/cldc|hp |hp-|htc |htc_|htc-|iemobile|kindle|midp|mmp|motorola|mobile|nokia|opera mini|opera |Googlebot-Mobile|YahooSeeker\\/M1A1-R2D2|android|iphone|ipod|mobi|palm|palmos|phone|pocket|portalmmm|ppc;|smartphone|sonyericsson|sqh|spv|symbian|treo|up.browser|up.link|vodafone|wap|windows ce|xda |xda_|xoom)/i', $_SERVER['HTTP_USER_AGENT'])) {
		return true;
	}
	else if (isset($_SERVER['HTTP_ACCEPT']) && (stripos($_SERVER['HTTP_ACCEPT'], 'text/html') === false) && ((stripos($_SERVER['HTTP_ACCEPT'], 'vnd.wap.wml') !== false) || (stripos($_SERVER['HTTP_ACCEPT'], 'vnd.wap.xhtml+xml') !== false))) {
		return true;
	}
	else {
		return false;
...
//PHP直接显示图片
function jieqi_getimg($img, $mime = '.jpg'){
    ...
    $quality = 100;
    if($imgExt == 'jpg') $quality = 9;        //输出质量,JPEG格式(0-100),PNG格式(0-9)
    $getImgInfo = "image{$imgExt}";
    $getImgInfo($imgInfo, null, $quality);    //2.将图像输出到浏览器或文件。如: imagepng ( resource $image )
    imagedestroy($imgInfo);
}

function jieqi_strstr($str, $array){
...
//    $str = htmlspecialchars($str, $quote_style);
 	if (JIEQI_COVER_DIR){
        ...
        $str = str_replace(array('  ', '&amp;', "\r\n"), array('&emsp;', '&', "</pre><pre class=\"note\">"), $str);
	}
...
function jieqi_htmlclickable($str, $styles = NULL)
{
	...
	$k++;
	$patterns[$k] = '/(?<=[^\\[\\]="\'\\/\\.;<>a-z0-9-]|^)([a-z]+?):\\/\\/([a-z0-9\\/\\-_+=.~!%@?#%&;:$\\' . "\xa9\xa6" . ']+)/i';
	...
	$k++;
	$patterns[$k] = '/(?<=[^\\[\\]="\'\\/\\.;<>]|^)www\\.([a-z0-9\\-]+)\\.([a-z0-9\\/\\-_+=.~!%@?#%&;:$\\' . "\xa9\xa6" . ']+)/i';
	...
	$k++;
	$patterns[$k] = '/(?<=[^\\[\\]="\'\\/\\.;<>]|^)([a-z0-9\\-_\\.]{3,})@([a-z0-9\\/\\-_+=.~!%@?#%&;:$\\' . "\xa9\xa6" . ']+)/i';
...
function jieqi_substr($str, $start, $length, $trimmarker = '...')
{
	...
	while (($i < $len) && ($l < $length)) {
		...
		if (128 < $asc) {
			...
			else if ((192 <= $asc) && ($asc <= 223)) {
				$cs = 2;
				$cl = 2;
			}
			else if ((224 <= $asc) && ($asc <= 239)) {
				$cs = 3;
				$cl = 2;
			}
			else if ((240 <= $asc) && ($asc <= 247)) {
				$cs = 4;
				$cl = 2;
			}
		}
...
function jieqi_strwords($str)
{
	if (defined('JIEQI_SYSTEM_SBYTE') && (0 < JIEQI_SYSTEM_SBYTE)) {
		return jieqi_count(preg_split('/\\s+/', $str, -1, PREG_SPLIT_NO_EMPTY));
	}
	else {
		$utf8 = (JIEQI_SYSTEM_CHARSET == 'utf-8' ? true : false);
		$len = strlen($str);
...
function jieqi_dbslashes($str, $use_slashes = false)
{
	...
	else if ((JIEQI_SYSTEM_CHARSET == 'big5') && (JIEQI_DB_CHARSET != 'big5')) {
		$str = strval($str);
		$l = strlen($str);
		$ret = '';
		$i = 0;
...
function jieqi_count($ary, $mode = COUNT_NORMAL)
{
    return isset($ary) && is_array($ary) ? count($ary, $mode) : 0;
}
...
function jieqi_readfile($file_name)
{
    ...
    $Oss = new AliUpload();
    $imgtype = array('.jpg', '.bmp', '.gif', '.png');
    $filetype = array('/configs/',JIEQI_CACHE_DIR,JIEQI_COMPILED_DIR,'.sql');
    $is_file = file_exists(strstr(str_replace("\\", "/", $file_name), $_SERVER['DOCUMENT_ROOT']) ? $file_name : ($_SERVER['DOCUMENT_ROOT'].$file_name));

    if (empty($is_file) && !empty($jieqiConfigs["system"]["downloadpic"]) && jieqi_strstr($file_name, $imgtype)) {
        $file = ltrim(str_replace(array($_SERVER['DOCUMENT_ROOT'].'/', JIEQI_URL.'/'), '', str_replace("\\", "/", $file_name)), '/');
        if ($Oss->IsExist($file)){
            $Oss->download($file, $_SERVER['DOCUMENT_ROOT'].$file_name);
		}
	}

	if (empty($is_file) && !empty($jieqiConfigs["system"]["oss"]) && !jieqi_strstr($file_name, $filetype)) {
        $file_name = ltrim(str_replace(array($_SERVER['DOCUMENT_ROOT'].'/', JIEQI_URL.'/'), '', str_replace("\\", "/", $file_name)), '/');

        $file_name = $Oss->ReadFile($file_name);
        if (jieqi_strstr($file_name, $imgtype)) {
        		return $file_name;
        	}
        	else{
...
function jieqi_writefile($file_name, $data, $method = 'wb')
{
    ...
    if (!empty($jieqiConfigs["system"]["oss"]) && !jieqi_strstr($file_name, array('/configs/',JIEQI_CACHE_DIR,JIEQI_COMPILED_DIR)) && !file_exists($file_name)) {
        $file_name = str_replace($_SERVER['DOCUMENT_ROOT'].'/', '', str_replace("\\", "/", $file_name));
        $ret = $Oss->upload($file_name, $data, 1);
    }
    else{
...
function jieqi_delfile($file_name)
{
	...
	if (!preg_match('/^(ftps?):\\/\\/([^:\\/]+):([^:\\/]*)@([0-9a-z\\-\\.]+)(:(\\d+))?([0-9a-z_\\-\\/\\.]*)/is', $file_name, $matches)) {
        if (!empty($jieqiConfigs["system"]["oss"])) {
            $file_name = str_replace($_SERVER['DOCUMENT_ROOT'].'/', '', str_replace("\\", "/", $file_name));
        	$Oss->DelFile($file_name);
		}
		return @(unlink($file_name));
	}
	else {
		include_once jieqi_path_lib("ftp/ftp.php");
...
function jieqi_delfolder($dirname, $flag = true)
{
	...
	if (!preg_match('/^(ftps?):\\/\\/([^:\\/]+):([^:\\/]*)@([0-9a-z\\-\\.]+)(:(\\d+))?([0-9a-z_\\-\\/\\.]*)/is', $dirname, $matches)) {
		...
		if ($jieqiConfigs["system"]["oss"]) {
            $to_file = str_replace($_SERVER['DOCUMENT_ROOT'].'/', '', str_replace("\\", "/", $dirname));
            $is_file = $Oss->IsExist($to_file);
            if ($is_file){
            	$Oss->DelFile($is_file);
			}
		}
...
function jieqi_copyfile($from_file, $to_file, $mode = 511, $move = false)
{
	...
	if (!preg_match('/^(ftps?):\\/\\/([^:\\/]+):([^:\\/]*)@([0-9a-z\\-\\.]+)(:(\\d+))?([0-9a-z_\\-\\/\\.]*)/is', $to_file, $matches)) {
		if (!empty($jieqiConfigs["system"]["oss"]) && !jieqi_strstr($to_file, array('/configs/',JIEQI_CACHE_DIR,JIEQI_COMPILED_DIR))) {
            $to_file = str_replace($_SERVER['DOCUMENT_ROOT'].'/', '', str_replace("\\", "/", $to_file));
			require_once jieqi_path_lib("Oss/Alioss/alioss.php");
			$Oss = new AliUpload();
			$ret = $Oss->upload($to_file, $from_file);
		}
		else{
...
{
    if (!preg_match('/^[\\w\\-_]+$/', $fname) || !preg_match('/^[\\w\\-_]+$/', $vname) || !preg_match('/^\\w*$/', $module)) {
        return false;
    }
    ...
    $file = jieqi_path_config($fname, $module);
    jieqi_checkdir(dirname($file), true);
    return jieqi_writefile($file, $varstring);
}
...
function jieqi_getconfigs($module = 'system', $fname = '', $vname = '')
{
    ...
    if (!preg_match('/^[\\w\\-_]+$/', $module . $fname . $vname)) {
        return false;
    }
    if (($vname == 'jieqiBlocks') && isset($jieqiBlocks)) {
        ...
        include_once jieqi_path_config($fname, $module);
        return ${$vname};
    }
}
