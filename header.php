...
function jieqi_get_block($blockconfig, $retflag = 0)
{
	...
	if ((($jieqiUsersStatus == JIEQI_GROUP_GUEST) && (0 < ($blockconfig["publish"] & 1))) || (($jieqiUsersStatus != JIEQI_GROUP_GUEST) && (0 < ($blockconfig["publish"] & 2)))) {
		...
		if (0 < $blockconfig["custom"]) {
			$blockfile = jieqi_path_inc("blocks/block_custom.php", $blockconfig["module"]);
		}
		else {
			$blockfile = jieqi_path_inc("blocks/" . trim($blockconfig["filename"]) . ".php", $blockconfig["module"]);
		}
		...
		if (($blockconfig["contenttype"] != JIEQI_CONTENT_PHP) && empty($blockconfig["hasvars"])) {
			...
			$templatefile = jieqi_path_template($templatefile, $blockconfig["module"]);
...
if (defined("JIEQI_DEVICE_FOR") && ((defined("JIEQI_DEVICE_CHECK") && (0 < intval(JIEQI_DEVICE_CHECK))) || (!defined("JIEQI_DEVICE_CHECK") && defined("JIEQI_MOBILE_LOCATION") && (0 < strlen(trim(JIEQI_MOBILE_LOCATION)))) || (!defined("JIEQI_DEVICE_CHECK") && defined("JIEQI_PC_LOCATION") && (0 < strlen(trim(JIEQI_PC_LOCATION)))))) {
...
if (!empty($_GET["fromuid"])) {
	@setcookie("jieqiChannel", $_GET["fromuid"], 0, "/", JIEQI_COOKIE_DOMAIN, 0);
}
...
if (!defined("JIEQI_ADMIN_PAGE")) {
	...
	if (!empty($_SERVER["HTTP_IF_NONE_MATCH"])) {
		...
		if (jieqi_count(1 < $etagary) && is_numeric($etagary[0]) && ($etagcheck == $etagary[1])) {
...
if ($jieqiUsersStatus == JIEQI_GROUP_GUEST) {
    ...
    $jieqiTpl->assign("jieqi_vipid", 0);
    $jieqiTpl->assign("jieqi_vipname", "");
    ...
    $jieqiTpl->assign("jieqi_vipid", $_SESSION["jieqiUserVipId"]);
    $jieqiTpl->assign("jieqi_vipname", $_SESSION["jieqiUserVipName"]);
...
$jieqi_api_sites = array(
	"weixin" => array("apiorder" => 2, "apititle" => LANG_APITITLE_WEIXIN, "publish" => 0),
	"qq"     => array("apiorder" => 1, "apititle" => LANG_APITITLE_QQ, "publish" => 0),
	"weibo"  => array("apiorder" => 4, "apititle" => LANG_APITITLE_WEIBO, "publish" => 0)
	);
...
if (!strstr($_SERVER['PHP_SELF'], "/admin/")) {
	jieqi_getconfigs("article", "gift", "jieqiGift");
	if(!empty($jieqiGift["article"])){
		foreach ($jieqiGift["article"] as $v => $s ) {
			$giftrows[$k] = jieqi_funtoarray("jieqi_htmlstr", $s);
			$giftrows[$k]["gid"] = $v;
			include_once jieqi_path_inc("include/badgefunction.php", "badge");
			$giftrows[$k]["icourl"] = getbadgeurl(5, $v, 0, true);
			if(empty($giftrows[$k]["icourl"])){
				$giftrows[$k]["icourl"] = JIEQI_URL."/images/picview.gif";
			}
			$price[$v] = $s['price'];
			$k++;
		}
	
		array_multisort($price,SORT_ASC,$giftrows);
		$jieqiTpl->assign_by_ref("giftrows", $giftrows);
	}
...
$honorid = $_SESSION['jieqiUserHonorid'];
...
if ($_SESSION['jieqiUserSet']["polldate"] != $today){
    $jieqiTpl->assign("jieqi_votes", $maxvote);
}
else{
    $jieqi_votes = $maxvote - $_SESSION['jieqiUserSet']['pollnum'];
    $jieqiTpl->assign("jieqi_votes", $jieqi_votes);
}
...
// $jieqiTpl->assign("jieqi_wap_url", JIEQI_MOBILE_LOCATION);
$jieqiTpl->assign("jieqi_time", JIEQI_NOW_TIME);
