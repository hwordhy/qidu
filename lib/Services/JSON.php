...
class Services_JSON
{
	...
	public function _encode($var)
	{
		...
		case 'string':
			...
			return '"' . $ascii . '"';
		case 'array':
			if (is_array($var) && jieqi_count($var) && (array_keys($var) !== range(0, sizeof($var) - 1))) {
		...
		default:
			...
			if (is_numeric($str)) {
				...
				for (; $c <= $strlen_chrs; ++$c) {
					...
					else if (($chrs[$c] == $top['delim']) && ($top['what'] == SERVICES_JSON_IN_STR) && (($this->strlen8($this->substr8($chrs, 0, $c)) - $this->strlen8(rtrim($this->substr8($chrs, 0, $c), '\\'))) % 2 != 1)) {
						array_pop($stk);
					}
