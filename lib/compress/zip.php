...
class JieqiZip
{
	...
	public function makezip($zipfile)
	{
		...
		$p_result_list = array();
		$v_header = array();
		$v_nb = jieqi_count($v_header_list);
		$filenum = jieqi_count($this->filelist);
		$stored_files = array();
		$root_path = "";
		$prevary = array();
		$curary = array();
		...
		for ($i = 0; $i < $filenum; $i++) {
			...
			if ($i == 0) {
				...
				else if (is_dir($this->filelist[$i])) {
				...
				if ((0 < $i) && ($root_path != "")) {
					...
					$prenum = jieqi_count($prevary);
					$curary = explode("/", $stored_files[$i]);
					$curnum = jieqi_count($curary);
					$j = 0;
					$root_path = "";
		...
		$filenum = jieqi_count($v_header_list);
		$v_count = 0;
	...
	{
		$v_offset = @ftell($this->zip_fd);
		$filenum = jieqi_count($this->headerlist);
		$v_count = 0;
