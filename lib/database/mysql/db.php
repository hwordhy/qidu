...
class JieqiMySQLDatabase extends JieqiObject
{
	...
	public function connect($master = false)
	{
		...
		if ($this->dbset["dbpconnect"] == 1) {
			if ($master) {
				$this->conn_m = @mysqli_pconnect($this->hosts[$hid]["dbhost"], $this->hosts[$hid]["dbuser"], $this->hosts[$hid]["dbpass"]);
				$this->conn = &$this->conn_m;
			}
			else {
				$this->conn_s = @mysqli_pconnect($this->hosts[$hid]["dbhost"], $this->hosts[$hid]["dbuser"], $this->hosts[$hid]["dbpass"]);
				$this->conn = &$this->conn_s;
			}
		}
		else if ($master) {
		...
		if (0 < strlen($this->hosts[$hid]["dbname"])) {
			if (!mysqli_select_db($this->hosts[$hid]["dbname"], $this->conn)) {
				jieqi_printfail("Can not select database!<br /><br />error: " . mysqli_error());
			}
		}
		*/
	...
	{
		$mysqli_version = mysqli_get_server_info($this->conn);

		if ("4.1" < $mysqli_version) {
			if (isset($this->dbset["dbcharset"])) {
				if ($this->dbset["dbcharset"] != "default") {
					@mysqli_query("SET character_set_connection=" . $this->dbset["dbcharset"] . ", character_set_results=" . $this->dbset["dbcharset"] . ", character_set_client=binary", $this->conn);
				}
			}
			else {
				@mysqli_query("SET character_set_connection=" . JIEQI_SYSTEM_CHARSET . ", character_set_results=" . JIEQI_SYSTEM_CHARSET . ", character_set_client=binary", $this->conn);
			}
		}

		if ("5.0" < $mysqli_version) {
	...
	{
		// var_dump(@mysqli_fetch_array($result, mysqli_ASSOC));die();
