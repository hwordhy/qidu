...
class JieqiPDODatabase extends JieqiObject
{
	...
	public function connect($master = false)
	{
		...
		if ($this->dbset["dbpconnect"] == 1) {
			if ($master) {
				try {
                    $this->conn_m = new PDO("mysql:host=" . $this->hosts[$hid]["dbhost"] . ";dbname=" . $this->hosts[$hid]["dbname"], $this->hosts[$hid]["dbuser"], $this->hosts[$hid]["dbpass"]);
                } catch (Exception $e) {
                ...
                try {
                    $this->conn_s = new PDO("mysql:host=" . $this->hosts[$hid]["dbhost"] . ";dbname=" . $this->hosts[$hid]["dbname"], $this->hosts[$hid]["dbuser"], $this->hosts[$hid]["dbpass"], array(PDO::ATTR_PERSISTENT => true));
                } catch (Exception $e) {
                    jieqi_printfail("Can not connect to database!<br /><br />error: " . $e->getMessage());
                }
