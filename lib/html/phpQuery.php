...
if (!function_exists('mb_substr_count')) {
    function mb_substr_jieqi_count($haystack, $needle)
    {
        return substr_jieqi_count($haystack, $needle);
    }
}
...
class DOMDocumentWrapper
{
	...
	{
		if (isset($nodes) && (jieqi_count($nodes) == 1) && $nodes[0] instanceof DOMDOCUMENT) {
			$nodes = NULL;
		}
		...
		if (isset($nodes)) {
			...
			if ($this->isXML && !$innerMarkup) {
				...
				self::debug('Getting markup, moving selected nodes (' . jieqi_count($loop) . ') to new DocumentFragment');
		...
		else if ($this->isDocumentFragment) {
...
class phpQueryEvents
{
	...
	static protected function setNode($documentID, $node)
	{
		...
		return phpQuery::$documents[$documentID]->eventsNodes[jieqi_count(phpQuery::$documents[$documentID]->eventsNodes) - 1];
	}
...
class phpQueryObject implements Iterator, ArrayAccess
{
	...
	protected function parseSelector($query)
	{
		...
		while ($i < $strlen) {
			...
			if ($this->isChar($c) || in_array($c, $tagChars)) {
				...
				$queries[] = array();
				$return = &$queries[jieqi_count($queries) - 1];
				$i++;
	...
	protected function matchClasses($class, $node)
	{
		if (mb_strpos($class, '.', 1)) {
			...
			$classesCount = jieqi_count($classes);
			$nodeClasses = explode(' ', $node->getAttribute('class'));
			$nodeClassesCount = jieqi_count($nodeClasses);

			if ($nodeClassesCount < $classesCount) {
				return false;
			}

			$diff = jieqi_count(array_diff($classes, $nodeClasses));
	...
	protected function runQuery($XQuery, $selector = NULL, $compare = NULL)
	{
		...
		foreach ($this->stack(array(1, 9, 13)) as $k => $stackNode ) {
			...
			if (phpQuery::$debug) {
				$this->debug('Matched ' . jieqi_count($debug) . ': ' . implode(', ', $debug));
			}
	...
	protected function pseudoClasses($class)
	{
		...
		case 'last':
			if ($this->elements) {
				$this->elements = array($this->elements[jieqi_count($this->elements) - 1]);
			}
	...
	public function filter($selectors, $_skipHistory = false)
	{
		...
		if ($_skipHistory) {
			...
			$this->debug('Stack length after filter(): ' . jieqi_count($finalStack));
			return $this->newInstance();
		}
	}
	...
	{
		return jieqi_count($this->elements);
	}
	...
	public function insert($target, $type)
	{
		...
		phpQuery::debug('From ' . jieqi_count($insertFrom) . '; To ' . jieqi_count($insertTo) . ' nodes');
	...
	public function text($text = NULL, $callback1 = NULL, $callback2 = NULL, $callback3 = NULL)
	{
		...
		foreach ($this->elements as $node ) {
			...
			if ((1 < jieqi_count($this->elements)) && $text) {
				$text .= "\n";
			}
