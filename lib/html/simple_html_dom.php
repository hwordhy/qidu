...
class simple_html_dom_node
{
	...
	public function dump($show_attr = true, $deep = 0)
	{
		...
		echo $lead . $this->tag;
		if ($show_attr && (0 < jieqi_count($this->attr))) {
			echo '(';
	...
	public function dump_node($echo = true)
	{
		...
		$string .= ' children: ' . jieqi_count($this->children);
		$string .= ' nodes: ' . jieqi_count($this->nodes);
	...
	{
		if (0 < jieqi_count($this->children)) {
	...
	{
		if (0 < ($count = jieqi_count($this->children))) {
	...
	public function next_sibling()
	{
		...
		$idx = 0;
		$count = jieqi_count($this->parent->children);
	...
	public function find($selector, $idx = NULL, $lowercase = false)
	{
		...
		if (($count = jieqi_count($selectors)) === 0) {
			return array();
		}
		...
		for ($c = 0; $c < $count; ++$c) {
			if (($levle = jieqi_count($selectors[$c])) === 0) {
				return array();
			}
...
class simple_html_dom
{
	...
	protected function remove_noise($pattern, $remove_tag = false)
	{
		...
		for ($i = $count - 1; -1 < $i; --$i) {
			$key = '___noise___' . sprintf('% 5d', jieqi_count($this->noise) + 1000);
