...
class JieqiTpl
{
	...
	public function __construct()
	{
		...
		if ( !defined('JIEQI_COVER_DIR') ) {
			$themeurl = JIEQI_THEME_ROOTURL . "/templates/themes/".JIEQI_THEME_NAME."/";
		} else {
			$themeurl = JIEQI_THEME_ROOTURL . "/cover/" . JIEQI_COVER_DIR . "/templates/themes/".JIEQI_THEME_NAME."/";
		}
		...
		$this->assign(array(
			"jieqi_url" => JIEQI_URL, 
			"jieqi_rootpath" => JIEQI_ROOT_PATH, 
			"jieqi_charset" => JIEQI_CHAR_SET, 
			"jieqi_version" => JIEQI_VERSION, 
			"jieqi_main_url" => JIEQI_MAIN_URL, 
			"jieqi_user_url" => JIEQI_USER_URL, 
			"jieqi_local_url" => JIEQI_LOCAL_URL, 
			"jieqi_theme" => JIEQI_THEME_NAME, 
			"jieqi_theme_rooturl" => JIEQI_THEME_ROOTURL, 
			"jieqi_themeurl" => $themeurl, 
			...
			"meta_author" => "http://www.jieqi.com (jieqi cms)", 
			"jieqi_host" => JIEQI_LOCAL_HOST, 
			"jieqi_time" => JIEQI_NOW_TIME, 
	...
	public function fetch($resource_name, $cache_id = NULL, $compile_id = NULL, $cache_time = NULL, $over_time = NULL, $display = false)
	{
		...
		else if (isset($_template_results)) {
			return $_template_results;
		}
	}
...
{
	if (!isset($str) || empty($str) || (is_array($str) && (jieqi_count($str) == 0))) {
		$str = $val;
	}
