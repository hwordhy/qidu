...
class XMLNode extends XMLText
{
		...
		while (++$i < $nChildren) {
			...
			case 'cdata':
				if ($lastelm != 'cdata') {
					...
					$children[jieqi_count($children) - 1]->nodeValue .= $vals[$i]['value'];
				}
			...
			case 'close':
				$nThisChildren = jieqi_count($children);
	...
	static public function createTextNode($value)
	{
		$tmp = new XMLText();
