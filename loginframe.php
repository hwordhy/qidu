...
else if ($jieqiUsersGroup == JIEQI_GROUP_GUEST) {
	$islogin = false;
}
else {
	$islogin = true;
...
if ($islogin) {
	...
	$jieqiTpl->display(jieqi_path_template("statusframe.html", "system"));
}
else {
	...
	$jieqiTpl->setCaching(0);
	$jieqiTpl->display(jieqi_path_template("loginframe.html", "system"));
}
