...
if (isset($_POST["act"]) && ($_POST["act"] == "clean")) {
	...
	$sortids = array();
	if (is_array($_POST["sortid"]) && (0 < jieqi_count($_POST["sortid"]))) {
	...
	while ($row = $query->getRow($res)) {
		...
		else if ($_POST["operate"] == "showarticle") {
	...
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/batchclean.html", "article");
