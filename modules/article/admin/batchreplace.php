...
if (isset($_REQUEST["act"])) {
	...
	else if (isset($_SESSION["tmp_txtsearch"])) {
	...
	else if (isset($_SESSION["tmp_txtreplace"])) {
...
$sortids = array();
if (is_array($_REQUEST["sortid"]) && (0 < jieqi_count($_REQUEST["sortid"]))) {
...
if (isset($_REQUEST["act"]) && ($_REQUEST["act"] == "replacewithid")) {
	...
	if ($_REQUEST["replacetype"] == 1) {
		...
		if (jieqi_count($fromary) != jieqi_count($toary)) {
	...
	if (!is_array($_REQUEST["flagary"]) || (jieqi_count($_REQUEST["flagary"]) < 1)) {
	...
	if (isset($_REQUEST["act"]) && ($_REQUEST["act"] == "replacewithtime")) {
		...
		$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/batchreplace.html", "article");
