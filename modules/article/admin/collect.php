...
if (!empty($_SERVER["SCRIPT_NAME"]) && (substr($_SERVER["SCRIPT_NAME"], -4) == ".php")) {
	...
	if (!empty($tmpary[jieqi_count($tmpary) - 1])) {
		$self_filename = $tmpary[jieqi_count($tmpary) - 1];
	}
}
...
case "newarticle":
	...
	else if (!jieqi_safestring($_REQUEST["articlename"])) {
	...
	if (empty($errtext)) {
		...
		if (!$article_handler->insert($newArticle)) {
			...
			if (!empty($_REQUEST["articleimage"]) && (0 < $imgflag)) {
				include_once jieqi_path_config("site_" . $jieqiCollectsite[$_REQUEST["siteid"]]["config"], "article");
				...
				if (!empty($tmpstr)) {
					$imagefile = $package->getDir("imagedir") . "/" . $id . "s" . jieqiConfigs["article"]["imagetype"];
...
case "collect":
	...
	if (empty($errtext)) {
		...
		if (array_key_exists($_REQUEST["siteid"], $jieqiCollectsite) && ($jieqiCollectsite[$_REQUEST["siteid"]]["enable"] == "1")) {
			include_once jieqi_path_inc("include/collectfunction.php", "article");
			include_once jieqi_path_config("site_" . $jieqiCollectsite[$_REQUEST["siteid"]]["config"], "article");
			...
			if (is_object($articleobj)) {
				...
				if (!empty($sort) && ($sort != "default") && isset($jieqiCollect["sortid"][$sort]) && (0 < intval($jieqiCollect["sortid"][$sort])) && ($articleobj->getVar("sortid", "n") == intval($jieqiCollect["sortid"]["default"]))) {
