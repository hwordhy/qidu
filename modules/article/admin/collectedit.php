...
if (empty($_REQUEST["config"]) || !preg_match("/^\w+$/", $_REQUEST["config"]) || !file_exists(jieqi_path_config("site_" . $_REQUEST["config"], "article"))) {
	jieqi_printfail($jieqiLang["article"]["rule_not_exists"]);
}

include_once jieqi_path_config("site_" . $_REQUEST["config"], "article");
...
case "edit":
	...
	jieqi_writefile(jieqi_path_config("site_" . $_POST["config"], "article"), $configstr);
	$siteid = -1;
	reset($jieqiCollectsite);

	while (list($k, $v) = func_new_each($jieqiCollectsite)) {
