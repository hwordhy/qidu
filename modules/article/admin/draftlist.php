...
if (isset($_POST["act"]) && !empty($_REQUEST["checkid"])) {
	...
	if (0 < jieqi_count($_REQUEST["checkid"])) {
		@set_time_limit(0);
		@session_write_close();
		jieqi_includedb();
		...
		case "delete":
			...
			while ($row = $query->getRow()) {
				...
				jieqi_sendmessage($row["posterid"], $row["poster"], sprintf($jieqiLang["article"]["draft_delmsg_title"], $row["articlename"], $row["chaptername"]), sprintf($jieqiLang["article"]["draft_delmsg_content"], $row["articlename"], $row["chaptername"]));
			}
...
$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/draftlist.html", "article");
