...
if (!file_exists(jieqi_path_config("site_" . $jieqiCollectsite[$_REQUEST["siteid"]]["config"], "article"))) {
...
case "collect":
	...
	echo sprintf($jieqiLang["article"]["page_collect_doing"], $jieqiCollect["sitename"], $jieqiCollect["listcollect"][$_REQUEST["collectname"]]["title"], $_REQUEST["collectpagenum"], jieqi_count($aidsary));
	ob_flush();
	flush();
	$cpoint = 1;
	...
	foreach ($aidsary as $v ) {
		...
		flush();
		$cpoint++;
		include jieqi_path_inc("include/updateone.php", "article");
	}
