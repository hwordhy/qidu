...
if ($_REQUEST["confirm"] != 1) {
	...
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/syncsite.html", "article");
...
if (empty($jieqiSites[$_REQUEST["siteid"]]["interface"]) || !preg_match("/^\w+$/i", $jieqiSites[$_REQUEST["siteid"]]["interface"]) || !is_dir(jieqi_path_inc("apic/" . $jieqiSites[$_REQUEST["siteid"]]["interface"], "article"))) {
...
include_once jieqi_path_inc("apic/" . $jieqiSites[$_REQUEST["siteid"]]["interface"] . "/apiclient.php", "article");
include_once jieqi_path_inc("include/funsync.php", "article");
...
if ((!empty($_REQUEST["syncid"]) && !empty($_REQUEST["order"])) || !empty($_REQUEST["sourceid"])) {
	...
	if (!empty($_REQUEST["sourceid"])) {
		...
		if ((jieqi_count($tmpary) < 2) || !is_numeric($tmpary[0]) || !is_numeric($tmpary[1])) {
			$sql = "DELETE FROM " . jieqi_dbprefix("article_syncsite") . " where syncid = " . $_REQUEST["syncid"];
			$query->execute($sql);
		...
		$sql = "SELECT articleid, articlename, lastupdate, display FROM " . jieqi_dbprefix("article_article") . " WHERE siteid = {$_REQUEST["siteid"]} AND sourceid = $sourceid LIMIT 0, 1";
		$query->execute($sql);
		$arow = $query->getRow();
		if (is_array($arow) && ($sourceupdate <= $arow["lastupdate"])) {
			$without_update = true;
	...
	if (!$without_update) {
		...
		$params = array("sid" => $_REQUEST["siteid"], "aid" => $sourceid);
		...
		if ($errstr === false) {
			...
			ob_flush();
			flush();
			$article_lastupdate = (empty($sync_article["lastupdate"]) ? time() : intval($sync_article["lastupdate"]));
			$sync_article["lastupdate"] = $article_lastupdate - 1;
			...
			if ($errstr === false) {
				...
				foreach ($sync_chapters as $sk => $sv ) {
					...
					if (isset($map_cids[$cidx])) {
						if ($old_chapters[$map_cids[$cidx]["key"]]["lastupdate"] < $sv["lastupdate"]) {
				...
				if (empty($reterror) && !empty($add_chaps)) {
					$batcount = jieqi_count($add_chaps);
					$batorder = 0;
					$batmax = 10;
					$batkey = 0;
					$scripts = array();
					...
					foreach ($add_chaps as $sk ) {
						if ($sync_chapters[$sk]["chaptertype"] == 0) {
							...
							$params[$batkey] = array("sid" => $_REQUEST["siteid"], "aid" => $sourceid, "cid" => $sync_chapters[$sk]["sourcecid"]);
							$skeys[$batkey] = $sk;
							$batkey++;
						}
				...
				if (0 < $up_chapternum) {
					...
					if (empty($del_chaps) && !empty($up_corders)) {
						$make_orders = array();
						$max_order = jieqi_count($sync_chapters);
	...
	$jieqiapi = new JieqiApiClient($jieqiSites[$_REQUEST["siteid"]]);
	$params = array("sid" => $_REQUEST["siteid"], "uptime" => 0);
