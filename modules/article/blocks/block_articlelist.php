...
class BlockArticleArticlelist extends JieqiBlock
{
	...
	public function getArticleSql()
	{
		...
		if (($this->exevars["isvip"] == 1) || ($this->exevars["isvip"] == 3)) {
			...
			if ($jieqiTop["article"][$this->exevars["order"]]["sort"] != "") {
				...
				$sortfield = preg_replace("/(^|[^a-z])size($|[^a-z])/i", "\$1allsize\$2", $jieqiTop["article"][$this->exevars["order"]]["sort"]);
				$sortfield = str_replace($orderfrom, $orderto, $sortfield);
				$sortfield = str_replace("allsize", "size", $sortfield);
			}
			...
			$sql .= " FROM " . jieqi_dbprefix("obook_obook") . " o LEFT JOIN " . jieqi_dbprefix("article_article") . " a ON o.articleid = a.articleid WHERE o.articleid > 0 AND a.display = 0 AND a.size > 0";
			...
			else if ($this->exevars["fullflag"] == 2) {
	...
	public function getArticleRow($v)
	{
		...
		if ($this->exevars["order"] == "size") {
			$row["ordervalue"] = jieqi_sizeformat($row["ordervalue"], "c");
		}
		else {
