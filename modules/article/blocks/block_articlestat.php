...
class BlockArticleArticlestat extends JieqiBlock
{
	...
	public function setContent($isreturn = false)
	{
		...
		$sql = "SELECT count(*) AS cot, sum(size) AS size, sum(daysize) AS daysize, sum(weeksize) AS weeksize, sum(monthsize) AS monthsize FROM " . jieqi_dbprefix("article_article") . " WHERE display = 0 AND size > 0";

		if (!empty($this->exevars["sortid"])) {
			$sortary = explode("|", $this->exevars["sortid"]);

			if (0 < jieqi_count($sortary)) {
				$ssql = "";
		...
		$sizeary = array("size", "daysize", "weeksize", "monthsize");

		foreach ($sizeary as $s ) {
			$articlestat[$s . "_c"] = jieqi_sizeformat($articlestat[$s], "c");
			$articlestat[$s . "_k"] = jieqi_sizeformat($articlestat[$s], "k");
			$articlestat[$s . "_w"] = jieqi_sizeformat($articlestat[$s], "w");
		}
