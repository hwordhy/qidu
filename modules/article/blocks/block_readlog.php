...
class BlockArticleReadlog extends JieqiBlock
{
	...
	public function setContent($isreturn = false)
	{
		...
		if ((jieqi_count($aids) < $this->exevars["listnum"]) && (0 < strlen($this->exevars["commend"]))) {
			$addnum = $this->exevars["listnum"] - jieqi_count($aids);
		...
		$i = 0;
		$maxrow = jieqi_count($articlerows);
		$sortrows = array();
