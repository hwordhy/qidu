...
class BlockArticleUbookcase extends JieqiBlock
{
	...
	public function setContent($isreturn = false)
	{
        ...
        jieqi_getconfigs("article", "option");
		...
		if (0 < $this->blockvars["cacheid"]) {
			...
			$criteria->setFields("c.*, a.articleid, a.lastupdate, a.articlename, a.authorid, a.author, a.sortid, a.typeid, a.lastchapterid, a.lastchapter, a.lastsummary, a.freetime, a.viptime, a.isvip, a.vipid, a.vipvolumeid, a.vipvolume, a.vipchapterid, a.vipchapter, a.vipsummary, a.imgflag");
			...
			while ($v = $bookcase_query->getObject()) {
                ...
                $bookcaserows[$k]["isvip"] = $jieqiOption['article']['isvip']['items'][$v->getVar("isvip")];
                $bookcaserows[$k]["fullflag"] = $jieqiOption['article']['fullflag']['items'][$v->getVar("fullflag")];
				$k++;
			}
		}
	...
	public function getCacheid()
	{
		...
		if (strval($this->exevars["uid"]) != "0") {
			...
			else if (is_numeric($this->exevars["uid"])) {
				$this->blockvars["cacheid"] = intval($this->exevars["uid"]);
			}
			else if (substr($this->exevars["uid"], 0, 1) == "\$") {
