...
case "update":
	...
	$chaptersize = (isset($_POST["chaptercontent"]) ? jieqi_strsize($_POST["chaptercontent"]) : 0);
	$chapterwords = jieqi_sizeformat($chaptersize, "c");
	...
	if (empty($errors)) {
		...
		if (0 < jieqi_count($oldattachary)) {
			...
			$chaptersize += jieqi_count($oldattachary) * $wordlen;
		}
		else {
			$attachinfo = "";
		}
		...
		if ((strlen($_POST["chaptercontent"]) == 0) && (0 < jieqi_count($oldattachary)) && ($_REQUEST["chaptertype"] == 0)) {
...
default:
	...
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("chapteredit.html", "article");
