...
case "post":
	...
	$_POST["num"] = intval(trim($_POST["num"]));

	if ($_POST["num"] <= 0) {
		$errtext .= $jieqiLang["article"]["giftbuy_over_zero"] . "<br />";
	}

	if (($_REQUEST["currency"] == "egold") && ($usermoney["emoney"] < ($jieqiGift["article"][$_REQUEST["type"]]["eprice"] * $_POST["num"]))) {
...
default:
	...
	if (0 < $jieqiGift["article"][$_REQUEST["type"]]["sprice"]) {
		...
		$jieqiTpl->assign("dft_money", $usermoney["emoney"]);
		$jieqiTpl->assign("dft_price", $jieqiGift["article"][$_REQUEST["type"]]["eprice"]);

		if (0 < $jieqiGift["article"][$_REQUEST["type"]]["eprice"]) {
			$jieqiTpl->assign("dft_maxnum", floor($usermoney["emoney"] / $jieqiGift["article"][$_REQUEST["type"]]["eprice"]));
		}
		else {
	...
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("giftbuy.html", "article");
