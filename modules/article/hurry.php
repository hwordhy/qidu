...
case "post":
	...
	$_POST["payegold"] = intval(trim($_POST["payegold"]));

	if ($_POST["payegold"] <= 0) {
		$errtext .= $jieqiLang["article"]["payegold_over_zero"] . "<br />";
	}
	else if ($_POST["payegold"] < $jieqiAction["article"]["hurry"]["paymin"]) {
		...
		else if ($usermoney["emoney"] < $_POST["payegold"]) {
			$errtext .= $jieqiLang["article"]["payegold_over_emoney"] . "<br />";
		}
	}

	$_POST["minsize"] = intval(trim($_POST["minsize"]));

	if ($_POST["minsize"] <= 0) {
		$errtext .= $jieqiLang["article"]["minsize_over_zero"] . "<br />";
	}
	...
	else if ($overtime <= JIEQI_NOW_TIME) {
	...
	if (empty($errtext)) {
		...
		$fieldrows["minsize"] = intval($_POST["minsize"]);
		...
		if (!$query->execute($sql)) {
			...
			$actions["review_content"] = sprintf($jieqiLang["article"]["hurry_review_content"], $_SESSION["jieqiUserName"], $_REQUEST["payegold"] . JIEQI_EGOLD_NAME, $actions["aname"], date("Y-m-d H:i", $fieldrows["overtime"]), $fieldrows["minsize"]);
			...
			$actions["message_content"] = sprintf($jieqiLang["article"]["hurry_message_content"], $_SESSION["jieqiUserName"], $_REQUEST["payegold"] . JIEQI_EGOLD_NAME, $actions["aname"], date("Y-m-d H:i", $fieldrows["overtime"]), $fieldrows["minsize"]);
...
default:
	...
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("hurry.html", "article");
