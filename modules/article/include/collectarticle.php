...
if (array_key_exists($_REQUEST["siteid"], $jieqiCollectsite) && ($jieqiCollectsite[$_REQUEST["siteid"]]["enable"] == "1")) {
	...
	if (empty($source)) {
		...
		if (empty($matchvar)) {
			...
			$j = 0;
			$k = 0;
			$chapternum = jieqi_count($chapterary);
			$volumenum = jieqi_count($volumeary);
			$volumename = "";
			...
			$fromnum = jieqi_count($fromrows);
			$tonum = jieqi_count($torows);
			...
			if (($fp < $fromnum) && ($tp <= $tonum)) {
				...
				$upordernum = jieqi_count($upcorders);

				if (jieqi_count($upcorders) < $maxchapterorder) {
					sort($upcorders);
				...
				if ($maxchapterorder <= jieqi_count($upcorders)) {
