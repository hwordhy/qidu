...
function jieqi_article_vars($article, $isfull = false, $format = "s")
{
	...
	$dwmary = array("visit" => "lastvisit", "vote" => "lastvote", "size" => "lastupdate", "down" => "lastdown", "flower" => "lastflower", "egg" => "lastegg", "vipvote" => "lastvipvote");
	...
	else if ($ret["lastupdate"] < $monthstart) {
	...
	foreach ($sizeary as $v ) {
		if (isset($ret[$v])) {
			$ret[$v . "_c"] = jieqi_sizeformat($ret[$v], "c");
			$ret[$v . "_k"] = jieqi_sizeformat($ret[$v], "k");
			$ret[$v . "_w"] = jieqi_sizeformat($ret[$v], "w");
		}
	}
