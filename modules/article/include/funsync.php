...
function jieqi_sync_chapterupdate($sync_chapter, $old_chapter)
{
	...
	$chaptersize = jieqi_strsize($sync_chapter["chaptercontent"]);
	$sync_chapter["size"] = $chaptersize;
	...
	if (($old_chapter["chaptertype"] == 0) && (0 < $old_chapter["isvip"])) {
		$fieldsmap = array("chaptername" => "chaptername", "size" => "size", "lastupdate" => "lastupdate", "summary" => "summary", "saleprice" => "saleprice", "display" => "display");
		$ofields = array();
	...
	if (($old_chapter["chaptertype"] == 0) && (($sync_chapter["size"] != $old_chapter["size"]) || ($sync_chapter["summary"] != $old_chapter["summary"]))) {
	...
	if ($old_chapter["chaptertype"] == 0) {
		if ($sync_chapter["isvip"] != $old_chapter["isvip"]) {
			if (!is_a($chapter_handler, "JieqiChapterHandler")) {
