...
function jieqi_sync_articleinfo($sync_article)
{
	...
	$criteria->add(new Criteria("articlename", '%'.$sync_article["articlename"].'%', 'LIKE'));
	$criteria->setLimit(1);
	...
	if (!is_object($article)) {
		...
		if (!empty($sync_article["posterid"])) {
			$article->setVar("posterid",$sync_article["posterid"]);
			$article->setVar("poster",$sync_article["poster"]);
		}
		else {
...
function jieqi_sync_getcover($cover)
{
	...
	}elseif(strpos($cover,"images.yueloo.com.cn")!==false){
		$ret = array("imgflag" => 2, "imgtype" => ".jpg");
		$ret["imgdata"] = jieqi_sync_geturlcontent($cover);
		return $ret;
	}
	else {
...
function jieqi_sync_chapterupdate($sync_chapter, $old_chapter,$format="json",$logfile='')
{
	global $jieqiModules;
	global $jieqiConfigs;
	global $jieqiLang;
	...
	//章节字数为空的话 计算章节字数
	if (($sync_chapter["chaptertype"] == 0) && ($sync_chapter["size"]==0)) {
		
		if (!empty($sync_chapter["url_content"])) {
			if($format=="xml"){
				$uc_content =json_decode(json_encode(simplexml_load_string(jieqi_sync_geturlcontent($sync_chapter["url_content"]), 'SimpleXMLElement', LIBXML_NOCDATA)), true);
			}else{
				$uc_content = json_decode(jieqi_sync_geturlcontent($sync_chapter["url_content"]),true);
			}
			

			if(array_key_exists("content",$uc_content)){
				$uc_content = $uc_content["content"];
			}elseif(array_key_exists("chapter_content",$uc_content)){
				$uc_content = $uc_content["chapter_content"];
			}elseif(array_key_exists("chaptercontent",$uc_content)){
				$uc_content = $uc_content["chaptercontent"];
			}else{
				$uc_content = $uc_content['data'];
				$uc_content = $uc_content["content"];
			}
			
			
			$uc_content = preg_replace("/'([^']*)'/", '‘${1}’',$uc_content);//替换英文单引号为中文单引号
			$uc_content = str_replace("'", "", $uc_content);

			/*****过滤多余的符号*****/
			$uc_content = str_replace("\n", "</p><p>", $uc_content);
			$uc_content = str_replace("§", "</p><p>", $uc_content);

			include_once jieqi_path_lib("text/texttypeset.php");
			$texttypeset = new TextTypeset();

			$uc_content = jieqi_textstr($uc_content, true);
							
			$uc_content = $texttypeset->doTypeset($uc_content);
			
			if ($uc_content === false) {
				file_put_contents($logfile, date('Y-m-d H:i:s').sprintf($jieqiLang["article"]["sync_chaptercontent_failure"], jieqi_htmlstr($sync_chapter["chaptername"]) . " URL:" . $sync_chapter["url_content"])."\r\n", FILE_APPEND);
			}else{
				$sync_chapter["size"] = strlen(preg_replace('/\\s/', '', $uc_content));
				$sync_chapter["saleprice"] = round($sync_chapter["size"]/3/$jieqiConfigs['obook']['wordsperegold']);
			}
	...
	if (isset($upfields["isvip"])) {
		if ($upfields["isvip"] == 0) {
			...
			if (is_object($ochapter)) {
				...
				if ($uc_content === false) {
					file_put_contents($logfile, date('Y-m-d H:i:s').sprintf($jieqiLang["article"]["sync_chaptercontent_failure"], jieqi_htmlstr($sync_chapter["chaptername"]) . "<br />URL:" . $sync_chapter["url_content"])."\r\n", FILE_APPEND);
...
function jieqi_sync_chapternew($sync_chapter, $article,$format="json",$logfile='')
{
	global $jieqiModules;
	global $jieqiConfigs;
	global $jieqiLang;
