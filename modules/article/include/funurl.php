...
function jieqi_url_article_article($aid, $type = "", $acode = "", $page = 1, $order = "")
{
	...
	case "index":
		...
		if (!in_array($order, array("", "0", "1", "asc", "desc"))) {
			$order = "";
		}
		...
		if (!empty($jieqiConfigs["article"]["fakearticle"]) || (!empty($jieqiConfigs["article"]["htmlurl"]) && (strpos($jieqiConfigs["article"]["htmlurl"], "<{\$aid}>") != false))) {
			...
			if ((0 < $jieqiConfigs["article"]["makehtml"]) && (JIEQI_CHAR_SET != JIEQI_SYSTEM_CHARSET) && !defined("JIEQI_NOCONVERT_CHAR") && (strpos($jieqiConfigs["article"]["fakearticle"], "<{\$newset}>") === false)) {
				...
				$repfrom = array("<{\$jieqi_url}>", "<{\$aid}>", "<{\$aid|subdirectory}>", "<{\$acode}>", "<{\$order}>", "<{\$order_s}>", "<{\$order_f}>", "<{\$isdesc}>", "<{\$newset}>");
				$repto = array(JIEQI_URL, $aid, jieqi_getsubdir($aid), $acode, $orderval, $order, $orderval, $isdesc, $newset);
			...
			if (($jieqiConfigs["article"]["makehtml"] == 0) || ((JIEQI_CHAR_SET != JIEQI_SYSTEM_CHARSET) && !defined("JIEQI_NOCONVERT_CHAR"))) {
				...
				return jieqi_uploadurl($jieqiConfigs["article"]["htmldir"], $jieqiConfigs["article"]["htmlurl"], "article", $article_static_url) . jieqi_getsubdir($aid) . "/" . $aid . "/index" . $order . "_<{\$page}>" . $jieqiConfigs["article"]["htmlfile"];
			}
			else {
				if (!empty($page) && (1 < $page)) {
					return jieqi_uploadurl($jieqiConfigs["article"]["htmldir"], $jieqiConfigs["article"]["htmlurl"], "article", $article_static_url) . jieqi_getsubdir($aid) . "/" . $aid . "/index" . $order . "_" . $page . $jieqiConfigs["article"]["htmlfile"];
					...
					return jieqi_uploadurl($jieqiConfigs["article"]["htmldir"], $jieqiConfigs["article"]["htmlurl"], "article", $article_static_url) . jieqi_getsubdir($aid) . "/" . $aid . "/index" . $order . $jieqiConfigs["article"]["htmlfile"];
				}
			}
		}
...
function jieqi_url_article_cover($aid, $type = "s", $flag = -1)
{
	...
	case "l":
		if (!empty($imageinfo["ltype"])) {
            $image = jieqi_uploadurl($jieqiConfigs["article"]["imagedir"], $jieqiConfigs["article"]["imageurl"], "article", $article_static_url) . jieqi_getsubdir($aid) . "/" . $aid . "/" . $aid . "l" . $imageinfo["ltype"];
			return jieqi_readfile($image);
		}
		else if (!empty($imageinfo["stype"])) {
            $image = jieqi_uploadurl($jieqiConfigs["article"]["imagedir"], $jieqiConfigs["article"]["imageurl"], "article", $article_static_url) . jieqi_getsubdir($aid) . "/" . $aid . "/" . $aid . "s" . $imageinfo["stype"];
            return jieqi_readfile($image);
		}
		else {
			return "";
...
function jieqi_url_article_articlefilter($page = 1, $filter = array())
{
	...
	if (!empty($jieqiConfigs["article"]["fakefilter"])) {
		...
		$ret = str_replace(array("<{\$sortid}>", "<{\$typeid}>", "<{\$initial}>", "<{\$order}>", "<{\$size}>", "<{\$update}>", "<{\$original}>", "<{\$progress}>", "<{\$isfull}>", "<{\$isvip}>", "<{\$rgroup}>"), "0", $ret);
...
function jieqi_url_article_reviews($page = 1, $aid)
{
    global $jieqiConfigs;
    global $article_dynamic_url;
    global $article_static_url;
    ...
    if (!empty($jieqiConfigs["article"]["fakereviews"])) {
		$use_fake = true;
		$repfrom = array("<{\$jieqi_url}>", "<{\$aid}>");
		$repto = array(JIEQI_URL, intval($aid));
		...
		$ret = trim(str_replace($repfrom, $repto, $jieqiConfigs["article"]["fakereviews"]));
...
function jieqi_url_article_reviewshow($page = 1, $tid)
{
    global $jieqiConfigs;
    global $article_dynamic_url;
    global $article_static_url;
    ...
    if (!empty($jieqiConfigs["article"]["fakereviewshow"])) {
        $use_fake = true;
        $repfrom = array("<{\$jieqi_url}>", "<{\$tid}>");
        $repto = array(JIEQI_URL, intval($tid));
        ...
        $ret = trim(str_replace($repfrom, $repto, $jieqiConfigs["article"]["fakereviewshow"]));
