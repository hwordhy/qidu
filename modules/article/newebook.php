...
if (empty($_REQUEST["action"]) || (strtolower($_REQUEST["action"]) !== "check")) {
	...
	$jieqiTpl->assign("sort", $jieqiSort);
	$jieqiTpl->setcaching(0);
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("newebook.html", "article");
...
$newArticle->setvar("chapters", jieqi_count($readlist));
...
$package->initpackage(array("id" => $newArticle->getvar("articleid", "n"), "title" => $newArticle->getvar("articlename", "n"), "creatorid" => $newArticle->getvar("authorid", "n"), "creator" => $newArticle->getvar("author", "n"), "subject" => $newArticle->getvar("keywords", "n"), "description" => $newArticle->getvar("intro", "n"), "publisher" => JIEQI_SITE_NAME, "contributorid" => $newArticle->getvar("posterid", "n"), "contributor" => $newArticle->getvar("poster", "n"), "sortid" => $newArticle->getvar("sortid", "n"), "typeid" => $newArticle->getvar("typeid", "n"), "articletype" => $newArticle->getvar("articletype", "n"), "permission" => $newArticle->getvar("permission", "n"), "firstflag" => $newArticle->getvar("firstflag", "n"), "fullflag" => $newArticle->getvar("fullflag", "n"), "imgflag" => $newArticle->getvar("imgflag", "n"), "power" => $newArticle->getvar("power", "n"), "display" => $newArticle->getvar("display", "n")), false);
$chapterid = 0;
$chaptername = "";
...
if (0 < $jieqiConfigs["article"]["makehtml"]) {
	foreach ($package->chapters as $k => $v ) {
		$package->makehtml($k + 1);
	}
