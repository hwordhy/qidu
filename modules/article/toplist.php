...
$jieqiTset["jieqi_contents_template"] = jieqi_path_template("toplist.html", "article");
...
if (!$content_used_cache) {
	...
	while ($v = $article_handler->getObject()) {
        ...
        if (!empty($_SESSION["jieqiUserId"])) {
            ...
            $criteria->add(new Criteria("articleid", $articlerows[$k]["articleid"]));
            $bookcase_handler->queryObjects($criteria);
            $bookcase = $bookcase_handler->getObject();
            if (is_object($bookcase)) {
                $articlerows[$k]["is_collection"] = 1;
            }
            else{
                $articlerows[$k]["is_collection"] = 0;
            }
        }
...
if (isset($_REQUEST["ajax_gets"]) && JIEQI_COVER_DIR == '') {
    echo json_encode(array("success" => true, "data" => array("list" => $articlerows, "totalpage" => $jumppage->totalpages), "message" => '获取成功'));
    return false;
