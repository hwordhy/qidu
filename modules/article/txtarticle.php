...
if ($package->loadOPF()) {
	...
	foreach ($package->chapters as $k => $chapter ) {
		if ($chapter["chaptertype"] == 1) {
			...
			echo jieqi_get_achapterc(array("articleid" => $package->id, "chapterid" => intval($chapter["chapterid"]), "isvip" => intval($chapter["isvip"]), "chaptertype" => 0, "display" => intval($chapter["display"]), "getformat" => "url"));
			ob_flush();
			flush();
		}
	}
