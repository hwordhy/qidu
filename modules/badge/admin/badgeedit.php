...
if (!isset($_REQUEST["btypeid"]) && isset($jieqiType[JIEQI_MODULE_NAME][$_REQUEST["btypeid"]]) && isset($_REQUEST["linkid"])) {
	jieqi_printfail(LANG_ERROR_PARAMETER);
}

$GLOBALS["_REQUEST"]["btypeid"] = intval($_REQUEST["btypeid"]);
$GLOBALS["_REQUEST"]["linkid"] = $_REQUEST["linkid"];
...
if ($_REQUEST["btypeid"] == 1) {
...
else if ($_REQUEST["btypeid"] == 4) {
	jieqi_getconfigs("article", "credit");

	if (!isset($jieqiCredit["article"][$_REQUEST["linkid"]])) {
		jieqi_printfail($jieqiLang["badge"]["badge_not_exists"]);
	}
	else {
		$badge_name = (isset($jieqiCredit["article"][$_REQUEST["linkid"]]["caption"]) ? $jieqiCredit["article"][$_REQUEST["linkid"]]["caption"] : $jieqiCredit["article"][$_REQUEST["linkid"]]["name"]["0"]);
	}

	$badge_maxnum = 0;
	$badge_linkid = 0;
}
else if ($_REQUEST["btypeid"] == 5) {
	jieqi_getconfigs("article", "gift");

	if (!isset($jieqiGift["article"][$_REQUEST["linkid"]])) {
		jieqi_printfail($jieqiLang["badge"]["badge_not_exists"]);
	}
	else {
		$badge_name = (isset($jieqiGift["article"][$_REQUEST["linkid"]]["caption"]) ? $jieqiGift["article"][$_REQUEST["linkid"]]["caption"] : $jieqiGift["article"][$_REQUEST["linkid"]]);
	}
	...
	include_once jieqi_path_inc("class/badge.php", "badge");
...
case "editbadge":
	...
	if (!empty($_FILES["badgeimage"]["name"])) {
		...
		if (preg_match("/\.(gif|jpg|jpeg|png|bmp)$/", $_FILES["badgeimage"]["name"])) {
