...
if ($_REQUEST["btypeid"] == 1) {
	...
	foreach ($jieqiCredit["article"] as $k => $v ) {
		$tmpary["badgeid"] = 0;
		$tmpary["btypeid"] = $_REQUEST["btypeid"];
		$tmpary["modname"] = "article";
	...
	foreach ($jieqiGift["article"] as $k => $v ) {
		$tmpary["badgeid"] = 0;
		$tmpary["btypeid"] = $_REQUEST["btypeid"];
		$tmpary["modname"] = "article";
	...
	if (($_REQUEST["action"] == "delete") && is_numeric($_REQUEST["badgeid"])) {
		...
		if (is_object($badge)) {
			...
			include_once jieqi_path_inc("class/award.php", "badge");
...
$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/badgelist.html", "badge");
