...
include_once jieqi_path_inc("include/funforum.php", "forum");
...
else if (!jieqi_forum_checkpower($forum, "authpost", true)) {
...
case "newpost":
	...
	$attachnum = jieqi_count($attachary);
	...
	if (empty($check_errors)) {
		...
		if (empty($_REQUEST["tid"])) {
			...
			if (!jieqi_topic_upreply($topic)) {
				jieqi_printfail($jieqiLang["system"]["post_topic_locked"]);
			}
		...
		if (0 < $attachnum) {
			include_once jieqi_path_inc("class/forumattachs.php", "forum");
		...
		if (!$post_handler->insert($newPost)) {
			...
			else if (!empty($jieqiConfigs["forum"]["scorereply"])) {
...
default:
	...
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("newpost.html", "forum");
