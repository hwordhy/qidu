...
if (!isset($_REQUEST["searchkey"]) || (strlen($_REQUEST["searchkey"]) == 0)) {
	...
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("search.html", "forum");
	...
	if (!empty($jieqiConfigs["forum"]["minsearchtime"]) && empty($_REQUEST["page"])) {
		...
		else if (!empty($jieqi_visit_time["jieqiForumsearchTime"])) {
	...
	$jieqiTset["jieqi_contents_template"] = jieqi_path_template("searchlist.html", "forum");
	include_once jieqi_path_common("header.php");
	...
	if (isset($_REQUEST["searchtype"]) && ($_REQUEST["searchtype"] == 2)) {
		...
		else if ($jieqiConfigs["forum"]["searchtype"] == 1) {
			$criteria->add(new Criteria("title", $_REQUEST["searchkey"] . "%", "like"));
		}
		else if ($jieqiConfigs["forum"]["searchtype"] == 2) {
