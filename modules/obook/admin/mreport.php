...
$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/mreport.html", "obook");
...
if (!empty($_REQUEST["act"]) && ($_REQUEST["act"] == "make") && !empty($_REQUEST["ryear"]) && !empty($_REQUEST["rmonth"])) {
	...
	while ($row = $query->getRow()) {
		if (isset($reportrows[$row["articleid"]])) {
			...
			$reportrows[$row["articleid"]]["presize"] = ($tmonths == $rmonths ? $row["monthsize"] : ($tmonths - $rmonths) == 1 ? $row["presize"] : 0);
...
if (!empty($_REQUEST["act"]) && in_array($_REQUEST["act"], array("paid", "unpay"))) {
	...
	if ((0 < jieqi_count($mrids)) && is_array($_REQUEST["paidtype"]) && (0 < jieqi_count($_REQUEST["paidtype"]))) {
		$upfields = array();
		$tmpvar = 0;
...
else if (!empty($_REQUEST["obookname"])) {
...
if (!empty($_REQUEST["format"])) {
	...
	include_once jieqi_path_inc("include/funexport.php", "system");
