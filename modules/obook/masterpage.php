...
$jieqiTset["jieqi_contents_template"] = jieqi_path_template("masterpage.html", "obook");
...
$sql = "SELECT count(*) AS cot, sum(sumegold) as sumegold, sum(sumtip) as sumtip, sum(sumgift) as sumgift, sum(sumemoney) as sumemoney, sum(paidemoney) as paidemoney FROM " . jieqi_dbprefix("obook_obook") . " WHERE authorid = $uid OR agentid = $uid OR posterid = $uid";
$query->execute($sql);
$row = $query->getRow();
