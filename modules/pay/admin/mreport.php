...
$jieqiTset["jieqi_contents_template"] = jieqi_path_template("admin/mreport.html", "pay");
...
if (!empty($_REQUEST["act"]) && ($_REQUEST["act"] == "make") && !empty($_REQUEST["ryear"]) && !empty($_REQUEST["rmonth"])) {
	...
	$reportrows = array();
	$sql = "SELECT siteid, paytype, moneytype, egoldtype, SUM(money) AS summoney, SUM(egold) AS sumegold FROM " . jieqi_dbprefix("pay_paylog") . " WHERE payflag > 0 AND buytime >= $starttime AND buytime < $endtime GROUP BY paytype";
	$query->execute($sql);
	$k = 0;
