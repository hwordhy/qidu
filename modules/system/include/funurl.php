...
function jieqi_url_system_avatar($uid, $size = "l", $type = -1, $retdft = true)
{
	...
	if (!empty($jieqiConfigs["system"]["oss"])) {
        $filedir = str_replace(JIEQI_URL, $_SERVER['DOCUMENT_ROOT'], $base_avatar);
        @mkdir($filedir,0777,true);
        if (!file_exists($filedir . "/" . $uid . $prefix)) {
            $file = ltrim(str_replace(array($_SERVER['DOCUMENT_ROOT'].'/', JIEQI_URL.'/'), '', str_replace("\\", "/", $filedir . "/" . $uid . $prefix)), '/');
            if ($Oss->IsExist($file)){
                $Oss->download($file, $filedir . "/" . $uid . $prefix);
            }
        }

        if (!file_exists($filedir . "/" . $uid . "s" . $prefix)) {
            $file = ltrim(str_replace(array($_SERVER['DOCUMENT_ROOT'].'/', JIEQI_URL.'/'), '', str_replace("\\", "/", $filedir . "/" . $uid . "s" . $prefix)), '/');
            if ($Oss->IsExist($file)){
                $Oss->download($file, $filedir . "/" . $uid . "s" . $prefix);
            }
        }

        if (!file_exists($filedir . "/" . $uid . "i" . $prefix)) {
            $file = ltrim(str_replace(array($_SERVER['DOCUMENT_ROOT'].'/', JIEQI_URL.'/'), '', str_replace("\\", "/", $filedir . "/" . $uid . "i" . $prefix)), '/');
            if ($Oss->IsExist($file)){
                $Oss->download($file, $filedir . "/" . $uid . "i" . $prefix);
            }
        }
	}
