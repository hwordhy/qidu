...

{
	...
	//2018.06.19  判断是否为推广会员 并写入到数据库

	if (!empty($_COOKIE["jieqiChannel"])) {

		$params["channel"] = $_COOKIE["jieqiChannel"];
    ...
    if (empty($params["emailrand"])) {
        ...
        $params["emailrand"] = trim($params["emailrand"]);
	...
	if (!defined("JIEQI_NO_CHECKCODE") && defined("JIEQI_LOGIN_CHECKCODE") && (0 < JIEQI_LOGIN_CHECKCODE) && (empty($params["checkcode"]) || empty($_SESSION["jieqiCheckCode"]) || (strtolower($params["checkcode"]) != strtolower($_SESSION["jieqiCheckCode"])))) {
	...
	//2018.04.10 验证Email验证码是否正确

    if ((defined("JIEQI_REGISTER_CHECKCODE") && JIEQI_REGISTER_CHECKCODE >= 2 && !defined("JIEQI_NO_CHECKCODE")) && (empty($params["emailrand"]) || empty($_SESSION["jieqiEmailCode"]) || (strtolower($params["emailrand"]) != strtolower($_SESSION["jieqiEmailCode"])))) {
...

{
	...
	if (!$newUser) {
		...
		if (!empty($jieqiConfigs["system"]["registeremail"]) && (0 < strlen($newUser->getVar("email", "n")))) {
			...
			$c_template = jieqi_path_template("emailregister.html", "system");
	...
	if ($islogin == 0) {
		...
		if (defined("JIEQI_USER_USERAPI") && (0 < JIEQI_USER_USERAPI) && (0 < $_SESSION["jieqiUserId"])) {
			...
			if (is_array($row) && !empty($row["uid"])) {
				...
				if (is_array($tmpvar) && (0 < jieqi_count($tmpvar))) {
...

{
	...
	if ( isset($params["qqgf"]) ) {
		$user->setVar("qqgf", $params["qqgf"]);
	}

	if ( isset($params["dygf"]) ) {
		$user->setVar("dygf", $params["dygf"]);
	}

	if ( isset($params["zyqdgf"]) ) {
		$user->setVar("zyqdgf", $params["zyqdgf"]);
	}

	if ( isset($params["dsfqdgf"]) ) {
		$user->setVar("dsfqdgf", $params["dsfqdgf"]);
	}

	if ( isset($params["bdgc"]) ) {
		$user->setVar("bdgc", $params["bdgc"]);
	}

	if ( isset($params["mdgc"]) ) {
		$user->setVar("mdgc", $params["mdgc"]);
	}
	...
	if ($params["adminlevel"] == 1) {
		...
		if (4 <= $params["adminlevel"]) {
			...
			if (isset($params["overtime"])) {
				...
				if (!is_numeric($params["overtime"]) && (0 < $params["overtime"])) {
