...
if (isset($_POST["act"]) && ($_POST["act"] == "post")) {
	...
	if (empty($errtext)) {
		include_once jieqi_path_inc("class/report.php", JIEQI_MODULE_NAME);
	...
	$jieqiTpl->setCaching(0);
	$jieqiTpl->assign("jieqi_contents", $jieqiTpl->fetch(jieqi_path_template("report.html", JIEQI_MODULE_NAME)));
	include_once jieqi_path_common("footer.php");
}
